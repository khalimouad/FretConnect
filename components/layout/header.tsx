...
export function Header({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  return (
    <>
      ...
      >
        <div className="flex flex-1 flex-col bg-white p-4 dark:bg-slate-950">
          ...
          <div className="mt-4 flex items-center gap-1 border-t border-slate-200 pt-4 dark:border-slate-800">
            <CurrencySelector dict={dict} />
            <ThemeToggle labels={themeLabels} />
            <LangSwitcher locale={locale} />
          </div>
          ...
          <div className="mt-auto flex flex-col gap-2 border-t border-slate-200 pt-4 dark:border-slate-800">
            <Link
              href={`/${locale}/login`}
              onClick={() => setMobileOpen(false)}
              className="rounded-lg px-3 py-2.5 text-center text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/10"
            >
              {dict.common.login}
            </Link>
            <Link
