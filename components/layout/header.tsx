...
function LangSwitcher({ locale }: { locale: Locale }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex cursor-pointer items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-sm font-medium text-slate-600 hover:bg-slate-100"
        aria-label="Language"
      >
      ...
      {open ? (
        <div className="absolute end-0 top-full z-50 mt-1 w-40 rounded-lg border border-slate-200 bg-white py-1 shadow-card">
          {locales.map((l) => (
            <Link
              key={l}
              href={`/${l}${rest}`}
              ...
              className={`block px-3 py-2 text-sm hover:bg-slate-50 ${
                l === locale ? "font-semibold text-brand-900" : "text-slate-600"
              }`}
            >
              {localeMeta[l].label}
            </Link>
...
function SpacesMenu({ locale, dict }: { locale: Locale; dict: Dictionary }) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex cursor-pointer items-center gap-1 rounded-lg px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-100"
      >
        {dict.nav.spaces}
        <ChevronDownIcon width={14} height={14} />
      </button>
      {open ? (
        <div className="absolute end-0 top-full z-50 mt-1 w-56 rounded-lg border border-slate-200 bg-white py-1 shadow-card">
