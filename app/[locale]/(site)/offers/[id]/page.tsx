    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6">
      ...
      <div className="mt-4 grid gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          <Card className="p-6 sm:p-8">
            ...
            <dl className="mt-6 grid gap-4 sm:grid-cols-3">
              {facts.map((f) => (
                <div key={f.label} className="rounded-lg bg-slate-50 p-4">
                  ...
                  <dd className="mt-1.5 font-semibold text-brand-950">{f.value}</dd>
                </div>
              ))}
            </dl>

            <div className="mt-6 flex items-baseline justify-between rounded-lg bg-accent-50 p-4 ring-1 ring-inset ring-accent-200">
              ...
              <span className="text-2xl font-bold text-accent-700">
                ...
                {offer.price ? (
                  <span className="ms-2 text-sm font-medium text-accent-600">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <div className="flex items-start gap-2.5">
                <MapPinIcon width={18} height={18} className="mt-0.5 text-emerald-600" />
                <div>
                  ...
                  <p className="font-medium text-slate-800">
                    {depCity?.name}
                    <span className="text-sm text-slate-400"> · {regionName(depCity?.regionId)}</span>
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2.5">
                <MapPinIcon width={18} height={18} className="mt-0.5 text-accent-600" />
                <div>
                  <p className="text-xs font-medium uppercase tracking-wide text-slate-400">
                    {dict.offer.arrival}
                  </p>
                  <p className="font-medium text-slate-800">
                    {arrCity?.name}
                    <span className="text-sm text-slate-400"> · {regionName(arrCity?.regionId)}</span>
                  </p>
                </div>
              </div>
            </div>
          ...
          {similar.length > 0 ? (
            <div>
              <h2 className="text-xl font-bold text-brand-950">{dict.offer.similar}</h2>
        ...
        <div className="space-y-4">
          <Card className="p-6">
            ...
            <p className="mt-1 flex items-center gap-2 text-lg font-bold text-brand-950">
              {offer.company.name}
            ...
            <dl className="mt-4 space-y-2 text-sm">
              <div className="flex justify-between gap-3">
                <dt className="text-slate-400">{dict.common.phone}</dt>
                <dd dir="ltr" className="font-medium text-slate-800">{offer.company.phone}</dd>
              </div>
              <div className="flex justify-between gap-3">
                <dt className="text-slate-400">{dict.common.email}</dt>
                <dd className="font-medium text-slate-800">{offer.company.email}</dd>
              </div>
            </dl>
